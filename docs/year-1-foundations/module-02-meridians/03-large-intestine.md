...
## ערוץ המעי הגס - שו יאנג מינג דה צ'אנג
...
| מאפיין | פרטים |
| ------------ | ---------------------------------------- |
| שם סיני | (Shou Yang Ming Da Chang Jing) |
| סוג | יאנג — יאנג מינג (Yang Ming) |
| גפה | יד (Hand) |
| איבר | מעי גס (Da Chang) |
...
* חוצה את הפילטרום (Ren Zhong / DU26) — כאן ערוצי ימין ושמאל מצטלבים
* LI20 של ימין נמצא ליד נחיר שמאל, ולהפך

#### 1.3 תפקודי איבר המעי הגס (Da Chang)

* **שולט בהעברה ובהוצאה** — מקבל פסולת מהמעי הדק ומוציא אותה
...
**שם סיני**: (Shang Yang) **משמעות השם**: "יאנג של המתכת" — שאנג = צליל המתכת; יאנג = ערוץ יאנג **קטגוריה מיוחדת**: נקודת ג'ינג-באר (Jing Xue) — אלמנט מתכת (Jin)
...
**שם סיני**: (Er Jian) **משמעות השם**: "המרווח השני" — הנקודה השנייה בערוץ, במרווח השני של האצבע **קטגוריה מיוחדת**: נקודת יינג-מעיין (Ying Xue) — אלמנט מים (Shui)
...
**שם סיני**: (San Jian) **משמעות השם**: "המרווח השלישי" — הנקודה השלישית בערוץ **קטגוריה מיוחדת**: נקודת שו-זרם (Shu Xue) — אלמנט עץ (Mu)
...
**שם סיני**: (He Gu) **משמעות השם**: "עמק המפגש" — הנקודה בעמק (הגומה) שבין עצמות המטקרפל הראשונה והשנייה **קטגוריה מיוחדת**: נקודת יואן-מקור (Yuan Xue); אחת מ-4 נקודות הפיקוד
...
* **כנקודת פיקוד**: "הפנים והפה — הה גו גוששת"
...
* **שילובים נפוצים**: LI4 + LR3 = "ארבע שערות" (Si Guan — מרגיע כאב, מפזר צ'י); LI4 + LU7 (הצטננות); LI4 + ST44 (כאב שיניים, חום בקיבה); LI4 + SP6 (לעידוד צירים — **אסור בהריון!**)

***

#### LI5 — יאנג שי

**שם סיני**: (Yang Xi) **משמעות השם**: "נחל היאנג" — נקודה בגומה (נחל) בצד היאנג של שורש כף היד **קטגוריה מיוחדת**: נקודת ג'ינג-נהר (Jing Xue) — אלמנט אש (Huo)
...
**שם סיני**: (Pian Li) **משמעות השם**: "מעבר צדדי" — ענף שסוטה הצידה מהערוץ הראשי **קטגוריה מיוחדת**: נקודת לואו מחברת (Luo Xue)
...
**שם סיני**: (Wen Liu) **משמעות השם**: "חום זורם" — נקודה שבה הצ'י נאסף וזורם בחמימות **קטגוריה מיוחדת**: נקודת שי-צלע (Xi Xue) — נקודת חירום/כאב חריף של הערוץ
...
**שם סיני**: (Xia Lian) **משמעות השם**: "צד תחתון" — הנקודה התחתונה מבין שתי נקודות "ליאן" באמה
...
**שם סיני**: (Shang Lian) **משמעות השם**: "צד עליון" — הנקודה העליונה מבין שתי נקודות "ליאן"
...
**שם סיני**: (Shou San Li) **משמעות השם**: "שלושה לי של היד" — 3 לי (=2 צון) מתחת למרפק; מקבילת היד של ST36 **קטגוריה מיוחדת**: נקודה חשובה לכאב מרפק ולזרוע
...
**שם סיני**: (Qu Chi) **משמעות השם**: "בריכה מעוקלת" — כשהמרפק מכופף, נוצרת גומה (בריכה) בקצה הקפל **קטגוריה מיוחדת**: נקודת הה-ים (He Xue) — אלמנט אדמה (Tu); נקודה מהחשובות באקופונקטורה
...
**שם סיני**: (Zhou Liao) **משמעות השם**: "שקע המרפק" — ז'או = מרפק; ליאו = שקע עצם
...
**שם סיני**: (Shou Wu Li) **משמעות השם**: "חמישה לי של היד" — 5 לי מעל המרפק
...
**שם סיני**: (Bi Nao) **משמעות השם**: "זרוע ושריר" — בי = זרוע; נאו = שריר הזרוע העליונה **קטגוריה מיוחדת**: נקודת מפגש עם ערוץ שלפוחית השתן (BL)
...
**שם סיני**: (Jian Yu) **משמעות השם**: "שקע הכתף" — ג'יאן = כתף; יו = שקע/בליטת עצם **קטגוריה מיוחדת**: נקודת מפגש עם ערוץ יאנג צ'יאו מאי
...
**שם סיני**: (Ju Gu) **משמעות השם**: "עצם גדולה" — ליד מפגש עצם הבריח עם ה-Acromion
...
**שם סיני**: (Tian Ding) **משמעות השם**: "קדרה שמיימית" — טיאן = שמיים; דינג = קדרה מפורסמת בתרבות הסינית
...
**שם סיני**: (Fu Tu) **משמעות השם**: "תומך בבליטה" — ליד תפוח האדם (הבליטה בגרון)
...
**שם סיני**: (He Liao) **משמעות השם**: "שקע התבואה" — הה = תבואה (מתייחס לשפם שנראה כשיבולים); ליאו = שקע
...
**שם סיני**: (Ying Xiang) **משמעות השם**: "מקבל את הניחוח" — הנקודה ליד האף, ופתיחתה מאפשרת לקבל ריחות **קטגוריה מיוחדת**: נקודת מפגש עם ערוץ הקיבה (ST); נקודת סיום הערוץ
...
| ------------------------ | ----- | ---------------------------- |
| ג'ינג-באר (Jing-Well) | LI1 | חירום, חום, אובדן הכרה |
| יינג-מעיין (Ying-Spring) | LI2 | פינוי חום |
| שו-זרם (Shu-Stream) | LI3 | כאבי מפרקים, רטיבות |
| יואן-מקור (Yuan-Source) | LI4 | שיכוך כאב, פנים ופה, הצטננות |
| ג'ינג-נהר (Jing-River) | LI5 | שורש כף יד, חום |
| לואו (Luo-Connecting) | LI6 | חיבור ל-LU, בצקות |
| שי-צלע (Xi-Cleft) | LI7 | כאב חריף, חירום |
| הה-ים (He-Sea) | LI11 | פינוי חום, בעיות עור, מרפק |

***

### 4. חמש נקודות שו של ערוץ המעי הגס

| סוג | נקודה | אלמנט | כיוון הזרימה |
| -------------------------- | ----- | -------- | ------------ |
| ג'ינג-באר (Jing-Well) | LI1 | מתכת | הצ'י "נובע" |
| יינג-מעיין (Ying-Spring) | LI2 | מים | הצ'י "זורם" |
| שו-זרם (Shu-Stream) | LI3 | עץ | הצ'י "מתחזק" |
| ג'ינג-נהר (Jing-River) | LI5 | אש | הצ'י "גדל" |
| הה-ים (He-Sea) | LI11 | אדמה | הצ'י "מתאסף" |

***

### 5. נקודות ה-Lower He-Sea

ערוץ המעי הגס הוא ערוץ יד, אך נקודת ה-Lower He-Sea שלו נמצאת **ברגל**:

* **ST37 (Shang Ju Xu)** — נקודת ה-Lower He-Sea של המעי הגס
