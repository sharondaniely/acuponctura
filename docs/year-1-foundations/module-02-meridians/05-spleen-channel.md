...
| פרט | תיאור |
| -------------------- | -------------------------- |
| **שם סיני** | (Zú Tài Yīn Pí Jīng) |
| **קיצור** | SP (Spleen) |
| **מספר נקודות** | 21 |
| **סוג** | ין (Yin) |
| **אלמנט** | אדמה (Tǔ) |
| **זוג פנימי-חיצוני** | קיבה (ST) |
| **שעות פעילות** | 09:00–11:00 |
| **כיוון זרימה** | מהרגל לחזה (עולה) |

#### 1.2 מסלול הערוץ

**מסלול חיצוני (Wài Xíng)**
...
**מסלול פנימי (Nèi Xíng)**

מהמפשעה, ענף פנימי חודר לחלל הבטן ומתחבר אל **הטחול** (Pí) - האיבר השייך לו, ואל **הקיבה** (Wèi) - איבר הפו המזווג. ממשיך כלפי מעלה, חוצה את הסרעפת, עולה לאורך הוושט ומגיע לשורש הלשון, שם מתפזר מתחתיה.

ענף נוסף מפריד מהקיבה, חוצה את הסרעפת ומתחבר ללב - שם הוא מצטרף לערוץ הלב (HT).

#### 1.3 זיווג פנימי-חיצוני

ערוץ הטחול (ין) מזווג עם ערוץ הקיבה (יאנג). שניהם שייכים לאלמנט האדמה (Tǔ):

* **הטחול** (ין) - מעביר ומטמיע (Yùn Huà) - אחראי על עיבוד והפצת תוצרי העיכול
* **הקיבה** (יאנג) - מקבלת ומרככת (Shòu Nà Fǔ Shú) - אחראית על קבלת מזון ועיכול ראשוני
...
1. **העברה והטמעה** (Yùn Huà): הטחול מעבד מזון ונוזלים, מפיק מהם את "תמצית הדגנים" (Shuǐ Gǔ Jīng Wēi), ומפיץ אותה לכל הגוף. זהו המקור העיקרי לצ'י ולדם שלאחר הלידה.
2. **שליטה בדם** (Tǒng Xuè): הטחול שומר שהדם יישאר בתוך כלי הדם ולא ידלוף מחוץ להם. חולשת טחול עלולה לגרום לדימומים.
3. **העלאת הצ'י** (Shēng Qīng): הטחול מעלה את הצ'י הצלול כלפי מעלה אל הריאות והלב. כאשר תפקוד זה נפגע, האיברים הפנימיים עלולים "לצנוח" (צניחת איברים).
4. **שליטה בשרירים ובגפיים** (Zhǔ Jī Ròu Sì Zhī): הטחול מזין את השרירים. חולשת טחול מתבטאת בחולשת שרירים ועייפות בגפיים.
5. **נפתח בפה, מתגלה בשפתיים** (Kāi Qiào Yú Kǒu, Qí Huá Zài Chún): מצב הטחול משתקף בתיאבון ובצבע השפתיים.

#### 2.2 פתולוגיות נפוצות

| פתולוגיה | סימנים עיקריים |
| ---------------------------- | --------------------------------------------------------- |
| **חסר צ'י הטחול** | עייפות, תיאבון ירוד, נפיחות אחרי אכילה, צואה רכה, חיוורון |
| **חסר יאנג הטחול** | כל הנ"ל + קרירות בבטן, שלשול עם מזון לא מעוכל, גפיים קרות |
| **הטחול לא שולט בדם** | דימומים כרוניים - דם בצואה, מחזור מרובה, חבורות בקלות |
| **צניחת צ'י הטחול** | צניחת איברים, שלשולים כרוניים, עייפות קשה, סחרחורת |
| **רטיבות-לחות בטחול** | כבדות בגוף, נפיחות, ליחה, שלשול, ציפוי לשון עבה ודביק |
| **רטיבות-חום בטחול** | צהבת, שלשול עם ריח, בטן נפוחה וכואבת, ציפוי צהוב ודביק |
...
* עוצרת דימומים (Zhǐ Xuè) - במיוחד דימום רחמי, דם בצואה, דם באף
* מרגיעה את הרוח (Ān Shén) - חלומות מטרידים, נדודי שינה, הזיות
* מחייה את ההכרה (Kāi Qiào)

**קטגוריה מיוחדת**: נקודת ג'ינג-באר (Jǐng) - אלמנט עץ; נקודת שורש
...
* מפנה חום (Qīng Rè)
* מחזקת את הטחול (Jiàn Pí) - כאבי בטן, נפיחות, עצירות או שלשול
* חום ללא הזעה, כאבי בטן, הקאות

**קטגוריה מיוחדת**: נקודת יינג-מעיין (Yíng) - אלמנט אש
...
* מחזקת את הטחול ואת הקיבה - נקודת מפתח לכל בעיות עיכול
* מפנה רטיבות (Huà Shī) - כבדות בגוף, נפיחות, שלשול
* מווסתת את הצ'י - כאבי בטן, בורבוריגמוס (קולות מעיים)
* כאבי גוף מרטיבות

**קטגוריה מיוחדת**: נקודת שו-זרם (Shū) - אלמנט אדמה; **נקודת יואן-מקור** (Yuán Xué)
...
* מפנה רטיבות
* מווסתת את צ'ונג מאי - בעיות גינקולוגיות, כאבי חזה
* מרגיעה את הלב והרוח
* כאבי לב, כאבי חזה

**קטגוריה מיוחדת**: **נקודת לואו-מחברת** (Luò Xué) - מתחברת לערוץ הקיבה; **נקודת פתיחה של צ'ונג מאי** (Chōng Mài) - כלי חודר (Penetrating Vessel)
...
**קטגוריה מיוחדת**: נקודת ג'ינג-נהר (Jīng) - אלמנט מתכת
...
**קטגוריה מיוחדת**: **נקודת מפגש** (Jiāo Huì Xué) של שלושת ערוצי הין של הרגל (טחול, כבד, כליות) - אחת הנקודות החשובות ביותר בגוף
...
* **נקודת מפתח לכאבי מחזור** (Tòng Jīng) - אחת הנקודות היעילות ביותר לדיסמנוריאה
* מווסתת מחזור חודשי - דימום לא סדיר, אמנוריאה
* מחזקת את הטחול ומפנה רטיבות
* שולטת בדימומים רחמיים
* בצקות, קושי בהשתנה

**קטגוריה מיוחדת**: **נקודת שי-צ'לפט** (Xì Xué) - נקודת הצטברות צ'י ודם; יעילה במיוחד למצבים חריפים וכאב
...
#### SP9 - ין לינג צ'ואן - Yīn Líng Quán
...
* **נקודת המפתח לסילוק רטיבות** (Lì Shī) - הנקודה היעילה ביותר לסילוק רטיבות מהגוף
* מחזקת את הטחול
* פותחת את מעברי המים - בצקות, מיימת, שתן מועט
* כאבי ברכיים (צד מדיאלי)
* דלקות בדרכי השתן
* שלשול, צהבת
* הפרשות לבנות מרובות

**קטגוריה מיוחדת**: נקודת הה-ים (Hé) - אלמנט מים
...
* **מזינה ומפעילה את הדם** - נקודת מפתח לכל מצבי דם
...
**קטגוריה מיוחדת**: נקודת מפגש עם ערוץ הכבד וין ווי מאי
...
**קטגוריה מיוחדת**: נקודת מפגש עם ין ווי מאי וערוץ הכבד
...
**קטגוריה מיוחדת**: נקודת מפגש עם ין ווי מאי
...
* כאבי חזה, כאבי שד
* מחסור בחלב אם (Cù Rǔ) - נקודה חשובה להגברת ייצור חלב
* שיעול, קוצר נשימה
* דלקת שד (מסטיטיס)
...
**קטגוריה מיוחדת**: **נקודת הלואו הגדולה של הטחול** (Pí Zhī Dà Luò) - מחברת את כל 15 ערוצי הלואו בגוף. נקודה ייחודית בכל מערכת המרידיאנים.
...
#### נקודות חמש האלמנטים (Wǔ Shū Xué)

| נקודה | סוג | אלמנט |
| ------------------- | -------------- | ----- |
| SP1 (ין באי) | ג'ינג-באר | עץ |
| SP2 (דא דו) | יינג-מעיין | אש |
| SP3 (טאי באי) | שו-זרם | אדמה |
| SP5 (שאנג צ'יו) | ג'ינג-נהר | מתכת |
| SP9 (ין לינג צ'ואן) | הה-ים | מים |

#### נקודות מיוחדות נוספות

| נקודה | קטגוריה |
| -------------- | ---------------------------------- |
| SP3 (טאי באי) | יואן - מקור |
| SP4 (גונג סון) | לואו - מחברת; פתיחת צ'ונג מאי |
| SP8 (די ג'י) | שי - צ'לפט |
| SP21 (דא באו) | לואו גדולה של הטחול |
...
* Deadman, P. _A Manual of Acupuncture_ - פרק ערוץ הטחול
* Maciocia, G. _The Foundations of Chinese Medicine_ - פרקי הטחול
* הואנג די ניי ג'ינג, לינג שו, פרק 10

***

> **נקודה למחשבה**: ערוץ הטחול הוא "אם הצ'י והדם שלאחר הלידה". חיזוק הטחול הוא הבסיס לכל טיפול ברפואה הסינית, משום שללא עיכול תקין והפקת צ'י ודם - אין לגוף ממה לחיות.
